...
type PostModule = { default: LifeEducationPost };
const metadataModules = import.meta.glob("./posts/*/meta.ts", { eager: true, import: "metadata" }) as Record<string, LifeEducationPostMeta>;
...
function getPostDirectorySlug(path: string): string {
  ...
  if (!match) throw new Error(`Unexpected LifeEducation post path: ${path}`);
  return match[1];
}
...
function assertUniqueSlugs(items: LifeEducationPostMeta[]): void {
  ...
  for (const post of items) {
    if (seen.has(post.slug)) throw new Error(`Duplicate LifeEducation post slug: ${post.slug}`);
    seen.add(post.slug);
  }
}
...
const loadedPosts = Object.entries(metadataModules).map(([path, metadata]) => {
  ...
  if (metadata.slug !== directorySlug) throw new Error(`Post slug "${metadata.slug}" must match folder "${directorySlug}".`);
  return metadata;
});
assertUniqueSlugs(loadedPosts);
const postLoaders = new Map(Object.entries(postModules).map(([path, loadModule]) => [getPostDirectorySlug(path), loadModule]));
...
export function getFeaturedPost(): LifeEducationPostMeta | undefined { return posts.find((post) => post.status === "Featured") ?? posts[0]; }
export function getPostBySlug(slug: string): LifeEducationPostMeta | undefined { return posts.find((post) => post.slug === slug); }
export async function loadPostBySlug(slug: string): Promise<LifeEducationPost | undefined> {
  ...
  if (!metadata || !loadModule) return undefined;
  return (await loadModule()).default;
}
export function getPostsByTopic(topic: string): LifeEducationPostMeta[] { return posts.filter((post) => post.topic === topic); }
export function getRecentPosts(limit?: number): LifeEducationPostMeta[] { return typeof limit === "number" ? posts.slice(0, limit) : posts; }
export function getPostHref(post: LifeEducationPostMeta | string): string { return `/posts/${typeof post === "string" ? post : post.slug}`; }

export function getRelatedPosts(post: LifeEducationPostMeta, limit = 3): LifeEducationPostMeta[] {
  const tags = new Set(post.tags ?? []);
  return posts
    .filter((candidate) => candidate.slug !== post.slug)
    .map((candidate) => {
      const tagOverlap = (candidate.tags ?? []).filter((tag) => tags.has(tag)).length;
      const topicBoost = candidate.topic && candidate.topic === post.topic ? 20 : 0;
      return { candidate, score: topicBoost + tagOverlap * 5 };
    })
...
export function getPostForDomain(domainNumber: string): LifeEducationPostMeta | undefined {
  return posts.find((post) => post.slug.startsWith(`domain-${domainNumber}-`));
}
...
export function formatPostDate(post: LifeEducationPostMeta): string {
  ...
  return new Intl.DateTimeFormat("en-US", { month: "long", day: "numeric", year: "numeric" }).format(new Date(`${post.publishedAt}T12:00:00`));
}
