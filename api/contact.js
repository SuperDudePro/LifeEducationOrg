...
export default async function handler(request, response) {
  ...
  if (rejectUnsafeRequest(request, response, { maxBytes: 8_192 })) return;
  ...
  const [ipRate, emailRate] = await Promise.all([
    durableRateLimit(`le:contact:ip:${clientIp(request)}`, { limit: 8, windowMs: 60 * 60 * 1000 }),
    email
      ? durableRateLimit(`le:contact:email:${email}`, { limit: 4, windowMs: 24 * 60 * 60 * 1000 })
